    ...
    <div className="p-10">
      <div id="pdf-content">
        {examResults.map((ogrenciObjesi, index) => {
          const ogrenciAdi =
            ogrenciObjesi.ogrenci?.ogr_ad_soyad || "Bilinmeyen Öğrenci";
          ...
          const netler = sinavlar.map(
            (sinavObjesi) => sinavObjesi?.sinav_net ?? 0
          );
          const puanlar = sinavlar.map(
            (sinavObjesi) => sinavObjesi?.sinav_puan ?? 0
          );
          const siralamalar = sinavlar.map(
            (sinavObjesi) => sinavObjesi?.sinav_siralama ?? 0
          );
